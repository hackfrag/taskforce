 ...
 * Changelog: 
 *  08.12.08: Version 1
 *  01.05.08: Version 2 (updated to jQuery UI 1.6 api and CSS framework)
 * 	
 * --------------------------------------------------------------------
 */
	...
	if(groups) {
		...
		jQuery(groups).each(function(h){
			...
			jQuery(this.options).each(function(i){
				var style = (int == selectOptions.length-1 || int == 0) ? 'style="display: none;"' : '' ;
				var labelText = (options.labelSrc == 'text') ? groupOpts[i].text : groupOpts[i].value;
				scale.append('<dd style="left:'+ leftVal(int) +'"><span class="ui-slider-label">'+ labelText +'</span><span class="ui-slider-tic ui-widget-content"'+ style +'></span></dd>');
				int++;
			});
		});
	}
